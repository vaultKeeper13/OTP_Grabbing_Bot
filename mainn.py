...
def send_welcome(message):
    ...
    if check_admin(id) == True:
        ...
        send = bot.send_message(message.chat.id,
                                "Access Not Authorized ❌\n\nJoin the Channel - @OpiumCarder / Contact Admin : @VaultKeeperAdmin")
...
def send_text(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "An error has occured , Join our Channel : @OpiumCarder")
...
def how_to_help(message):
    bot.send_message(message.chat.id, "• Contact @VaultKeeperAdmin / Join our Channel : @OpiumCarder 👑\n\n• Use /faq for more help")


@bot.message_handler(commands=['faq'])
def how_faq(message):
    bot.send_message(message.chat.id, "• Please Join @OpiumCarder for tutorial videos, and more helpful information . \n\n• Send vouches to @VaultKeeperAdmin \n ")
...
def handle_statuscallbacks(userid):
    ...
    if 'CallStatus' in request.values:
        ...
        try:
            ...
            bot.send_message(chat_id1, "Sorry an error has occured\nContact Admin @VaultKeeperAdmin / Join Channel : @OpiumCarder")
        else:
            return 'ok'
    else:
        return 'ok'
